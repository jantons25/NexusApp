...
function ReposicionesList({
  ...
  const lotesFiltrados = lotesOrdenados.filter((lote) => {
    return lote.reposiciones.some((repo) => {
      const productoId =
      ...
      const fechaFormateada = new Date(repo.createdAt).toLocaleDateString();
      ...
      <table className="w-full table-auto text-sm text-left text-gray-700">
        <thead className="bg-gray-100 text-xs uppercase text-gray-500">
          <tr>
            ...
            <th className="px-6 py-2 text-center">Habitación</th>
            <th className="px-6 py-2 text-center">Responsable</th>
